...
def extractRegion(image, pred_mask):
    ...
    seg_map = OverLayLabelOnImage(image, pred_mask).astype('uint8')
    ###################bbox##################
    crops, classes = [], []

    minAreas = [0.001, 0.001, 0.02, 0.003, 0.0004, 0.0004, 0.0003, 0.0005, 0.02, 0.02, 0.001, 0.001, 0.001, 0.001, 0.02]
    pred_masks = [(pred_mask == v) for v in range(15)]
    for i in range(1, 15):
        minArea = image.shape[:2][0] * image.shape[:2][1] * minAreas[i]
        ...
        for idx, cnt in enumerate(contours):
            ...
            carea = cropped.shape[:2][0] * cropped.shape[:2][1]
            if (carea > minArea):
                crops.append(cropped)
                classes.append(TOTAL_CLASSES[i])
    return crops, classes, seg_map


def saveImageRegions(images, classes, seg_map, result_dir='out'):
    os.makedirs(result_dir, exist_ok=True)
    cv2.imwrite(os.path.join(result_dir, 'seg_map.png'), seg_map)
    for idx, cnt in enumerate(images):
        cv2.imwrite(os.path.join(result_dir, classes[idx] + '_' + str(idx) + '.png'), cnt)
...
images, classes, sg = extractRegion(imcpy, pr_mask)
saveImageRegions(images, classes, sg)